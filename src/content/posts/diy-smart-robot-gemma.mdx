...
description: "Wiring a small on-device model into something that moves safely: split-brain architecture, firmware loops, and local perception."
date: 2026-06-09
draft: false
...
![A finished reference build: small rover, local AI workstation, camera, distance sensor, speaker, and safe wiring.](/images/cover.png)
...
### 2. Obstacle rover
Give the model only a few safe actions: `stop`, `turn_left`, `turn_right`, `forward_slowly`. Use short movements, then stop, re-sense, and decide again.
...
*For the complete visual booklet, browse the [DIY Smart Robot Research Booklet](/research/diy-smart-robot-gemma/).*
